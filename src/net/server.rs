...
impl TonboRpc for TonboService {
    ...
    type ScanStream = Pin<Box<dyn Stream<Item = Result<ScanResp, Status>> + Send + 'static>>;
    ...
    async fn scan(&self, request: Request<ScanReq>) -> Result<Response<Self::ScanStream>, Status> {
        ...
        let stream = stream! {
            ...
            let mut stream = std::pin::pin!(this
        ...
        Ok(Response::new(Box::pin(stream)
            as Pin<
                Box<dyn Stream<Item = Result<ScanResp, Status>> + Send + 'static>,
            >))
    }

    async fn insert(&self, request: Request<InsertReq>) -> Result<Response<Empty>, Status> {
        let mut req = request.into_inner();
        let record = Vec::<Column>::decode(&mut Cursor::new(&mut req.record))
            .await
            .map_err(|e| Status::new(Code::Internal, e.to_string()))?;
